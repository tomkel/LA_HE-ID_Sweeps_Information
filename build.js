    ...
    .map((asset) => {
      ...
      loadingTask.promise.then((doc) => {
        ...
        const loadPage = function (pageNum) {
          return doc.getPage(pageNum).then((page) => page.getTextContent().then((content) => {
            const sameCell = content.items.reduce((acc, item) => {
              if (acc.previous === null) {
                return {
                  results: acc.results, row: acc.row, previous: item, rowY: item.transform[5],
                }
              }
              ...
              if (previousX === currentX || withInDelta(1, previousX + acc.previous.width, currentX)) {
                return {
                  results: acc.results, row: acc.row, previous: { str: acc.previous.str + item.str, transform: acc.previous.transform, width: acc.previous.width }, rowY: acc.rowY,
                }
              } if (withIn(acc.rowY, currentY) || currentY > acc.rowY) {
                acc.row.push({ text: acc.previous.str, start: previousX, end: previousX + acc.previous.width })

                return {
                  results: acc.results, row: acc.row, previous: item, rowY: acc.rowY,
                }
              }
              acc.row.push({ text: acc.previous.str, start: previousX, end: previousX + acc.previous.width })
              acc.results.push(acc.row)
              return {
                results: acc.results, row: [], previous: item, rowY: currentY,
              }
            }, {
        ...
        function parseData(rows) {
          ...
          return {
            dateId, date, activities: data, invalid: invalidRows, isFuture,
          }
        }
        ...
        lastPromise.then(() => {
          ...
          const fileData = JSON.stringify({
            date: data.date, activities: data.activities, url: asset.url, name: asset.name,
          })
